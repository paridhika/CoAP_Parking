...
class PutClientThread(threading.Thread):
	...
	def store_time(self,location):
		...
		filled_slotsdict[location] = time.time() + wait
	def run(self):
		f1 = open('Results/run1/80/put_simulation_service_time_80_run1.csv', 'wt')
		f = open('Results/run1/80/put_simulation_result_80_run1.csv', 'wt')
		writer = csv.writer(f)
...
class GetClientThread(threading.Thread):
	def __init__(self,get_mean,mean_hold,count):
		...
		self.count = count
	def run(self):
		f1 = open('Results/run1/80/get_simulation_service_time_80_run1.csv', 'wt')
		f = open('Results/run1/80/get_simulation_result_80_run1.csv', 'wt')
		writer = csv.writer(f)
		service = csv.writer(f1)
		writer.writerow( ('Arrival Rate', 'Service Time','Throughput') )
		service.writerow( ('Mean', 'Count', 'Service Time') )
...
class DeleteClientThread(threading.Thread):
	def __init__(self,mean_hold,count):
		...
		self.count = count
	def run(self):
		f1 = open('Results/run1/80/delete_simulation_service_time_80_run1.csv', 'wt')
		f = open('Results/run1/80/delete_simulation_result_80_run1.csv', 'wt')
		writer = csv.writer(f)
		service = csv.writer(f1)
		writer.writerow( ('Arrival Rate', 'Service Time','Throughput') )
		service.writerow( ('Mean', 'Count', 'Service Time') )
